...
namespace StarWarsEF
{
    internal class Extractor
    {
        public List<string> Number1()
        {
            return planets.Where(p => p.Name.StartsWith("M"))
                .Select(p => p.Name).ToList();
        }
        public List<string> Number2()
        {
            return planets.Where(p => p.Name.ToLower().Contains("y"))
                .Select (p => p.Name).ToList();
        }
        public List<string> Number3()
        {
            return planets.Where(p => p.Name.Length > 9 && p.Name.Length < 15)
                .Select(p => p.Name).ToList();
        }
        public List<string> Number4()
        {
            return planets.Where(p => p.Name.EndsWith("e") && p.Name.IndexOf("a") == 1)
                .Select(p => p.Name).ToList();
        }
        public List<string> Number5()
        {
            return planets.Where(p => p.RotationPeriod > 40)
                .OrderBy(p => p.RotationPeriod)
                .Select(p => p.Name).ToList();
        }
        public List<string> Number6()
        {
            return planets.Where(p => p.RotationPeriod > 10 && p.RotationPeriod < 20)
                .OrderBy(p => p.Name)
                .Select(p => p.Name).ToList();
        }
        public List<string> Number7()
        {
            return planets.Where(p => p.RotationPeriod > 30)
                .OrderBy(p => p.Name).ThenBy(p => p.RotationPeriod)
                .Select(p => p.Name).ToList();

        }
        public List<string> Number8()
        {
            return planets.Where(p => (p.RotationPeriod < 30 || p.SurfaceWater > 50) && p.Name.Contains("ba"))
                .OrderBy(p => p.Name)
                .ThenBy(p => p.SurfaceWater)
                .ThenBy(p => p.RotationPeriod)
                .Select(p => p.Name).ToList();
        }
        public List<string> Number9()
        {
            return planets.Where(p => p.SurfaceWater > 0)
                .OrderByDescending(p => p.SurfaceWater)
                .Select(p => p.Name).ToList();
        }
        public List<string> Number10()
        {
            return planets.Where(p => p.Population > 0 && p.Diameter > 0)
                .OrderBy(p => (4 * Math.PI * Math.Pow((p.Diameter / 2), 2)) / p.Population)
                .Select(p => p.Name).ToList();

        }
        public List<string> Number11()
        {
            return planets.Except(planets.Where(p => p.RotationPeriod > 0))
                .Select(p => p.Name).ToList();
        }
        public List<string> Number12()
        {
            return planets.Where(p => p.Name.StartsWith("A") || p.Name.EndsWith("s"))
                .Union(planets.Where(p => p.Terrain != null && p.Terrain.Contains("rainforests")))
                .Select(p => p.Name).ToList();
        }
        public List<string> Number13()
        {
            return planets.Where(p => p.Terrain != null && (p.Terrain.Contains("desert") || p.Terrain.Contains("deserts")))
                .Select(p => p.Name).ToList();
        }
        public List<string> Number14()
        {
            return planets.Where(p => p.Terrain != null && (p.Terrain.Contains("swamp") || p.Terrain.Contains("swamps")))
                .OrderBy(p => p.RotationPeriod)
                .ThenBy(p => p.Name)
                .Select(p => p.Name).ToList();

        }
        public List<string> Number15()
        {
            return planets.Where(p => Regex.IsMatch(p.Name, @"(aa|ee|ii|oo|uu|yy)"))
                .Select(p => p.Name).ToList();
        }
        public List<string> Number16()
        {
            return planets.Where(p => Regex.IsMatch(p.Name, @"(kk|ll|rr|nn)"))
